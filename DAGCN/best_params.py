...
def run(args):
    ...
    for num_run in range(args.num_runs):
        ...
        trainer = pl.Trainer(
            ...
            devices=devices,  # 根据是否使用 GPU 或 CPU 设置 devices
        )
...
def find():
    ...
    # 直接使用已经下载和处理过的数据集路径
    if args.dataset in ['chameleon', 'squirrel', 'directed_roman_empire', "directed_amazon_ratings", "directed_questions", 'cora_ml', 'citeseer_full']:
    ...
    num_layers_range = [4, 5, 6]
    jk_types = ["max", "cat"]
    patience_values = [400]
    alphas = [0, 0.25, 0.5, 0.75, 1]
    learn_alpha_values = [False]
            ...
            for num_layers in num_layers_range:
                for jk in jk_types:
                    for patience in patience_values:
                        for alpha in alphas:
                            for learn_alpha in learn_alpha_values:
