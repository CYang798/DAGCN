...
def get_conv(conv_type, input_dim, output_dim, beta1):
    if conv_type == "gcn":
        return GCNConv(input_dim, output_dim, add_self_loops=False)
    elif conv_type == "dir-gcn":
        return DirGCNConv(input_dim, output_dim, beta1)
    else:
        raise ValueError(f"Convolution type {conv_type} not supported")


class DirGCNConv(nn.Module):
    def __init__(self, input_dim, output_dim, beta1):
        ...
        self.adj_norm = None
        self.adj_second_order_norm = None
    ...
    def compute_second_order_proximity(self, edge_index, num_nodes):
        ...
        self.adj_second_order_norm = get_norm_adj(adj_square, norm="sym")
    ...
    def forward(self, x, edge_index):
        if self.adj_norm is None:
            ...
            adj = SparseTensor(row=row, col=col, sparse_sizes=(num_nodes, num_nodes))
            self.adj_norm = get_norm_adj(adj, norm="sym")

            # 计算并归一化二阶邻接矩阵 A^2
            self.compute_second_order_proximity(edge_index, num_nodes)

        # 一阶传播
        first_order = self.lin_src_to_dst(self.adj_norm @ x)

        # 二阶传播
        second_order = self.lin_dst_to_src(self.adj_second_order_norm @ x)

        # 加权
        return self.beta1 * first_order + (1 - self.beta1) * second_order
...
class GNN(nn.Module):
    def __init__(self,
        ...
        if num_layers == 1:
            self.convs = ModuleList([get_conv(conv_type, num_features, output_dim, self.beta1)])
        else:
            self.convs = ModuleList([get_conv(conv_type, num_features, hidden_dim, self.beta1)])
            for _ in range(num_layers - 2):
                self.convs.append(get_conv(conv_type, hidden_dim, hidden_dim, self.beta1))
            self.convs.append(get_conv(conv_type, hidden_dim, output_dim, self.beta1))
